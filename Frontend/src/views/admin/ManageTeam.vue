...
export default {
  components: {},
  data() { return { teamMembers: [], isEditing: false, isLoading: false, notification: { show: false, message: '', type: 'success' }, selectedPhotoFile: null, selectedHeaderFile: null, photoPreviewUrl: null, headerPreviewUrl: null, form: this.getInitialForm(), }; },
  computed: { prestasi_string: { get() { return ((this.form.bio_data && this.form.bio_data.prestasi) || []).join('\n'); }, set(value) { if (!this.form.bio_data) { this.form.bio_data = {}; } this.form.bio_data.prestasi = value.split('\n').filter(p => p.trim() !== ''); } } },
  methods: {
    getInitialForm() { return { id: null, name: '', position: '', photo_path: null, header_photo_path: null, social_links: [], bio_data: { jurusan: '', ttl: '', hobi: '', sambutan: '', visi_misi: '', prestasi: [] }, order: 0, }; },
    showNotification(message, type = 'success') { this.notification = { show: true, message, type }; setTimeout(() => { this.notification.show = false; }, 3000); },
    getFullImageUrl(path, type = 'profile') { const placeholder = type === 'header' ? 'https://placehold.co/1920x500/E2E8F0/A0AEC0?text=Latar' : 'https://placehold.co/120x120/E2E8F0/A0AEC0?text=Foto'; if (!path) return placeholder; if (path.startsWith('http') || path.startsWith('blob')) return path; return `${API_BASE_URL}/storage/${path}`; },
    handleFileChange(event, type) { const file = event.target.files[0]; if (!file) return; const previewUrl = URL.createObjectURL(file); if (type === 'photo') { this.selectedPhotoFile = file; this.photoPreviewUrl = previewUrl; } else if (type === 'header') { this.selectedHeaderFile = file; this.headerPreviewUrl = previewUrl; } },
    async fetchTeamMembers() { try { const response = await axios.get(API_URL); this.teamMembers = response.data.sort((a, b) => a.order - b.order); } catch (error) { console.error("Gagal ambil anggota:", error); this.showNotification('Gagal muat data.', 'error'); } },
    async submitForm() { this.isLoading = true; const formData = new FormData(); formData.append('name', this.form.name || ''); formData.append('position', this.form.position || ''); const orderValue = Number.isInteger(this.form.order) && this.form.order >= 0 ? this.form.order : 0; formData.append('order', orderValue); formData.append('social_links', JSON.stringify(Array.isArray(this.form.social_links) ? this.form.social_links.filter(link => link.platform && link.url) : [])); formData.append('bio_data', JSON.stringify(this.form.bio_data || {})); if (this.selectedPhotoFile) formData.append('photo_file', this.selectedPhotoFile); if (this.selectedHeaderFile) formData.append('header_photo_file', this.selectedHeaderFile); try { if (this.isEditing) { formData.append('_method', 'PUT'); await axios.post(`${API_URL}/${this.form.id}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } }); this.showNotification('Berhasil diperbarui!'); } else { await axios.post(API_URL, formData, { headers: { 'Content-Type': 'multipart/form-data' } }); this.showNotification('Berhasil ditambah!'); } await this.fetchTeamMembers(); this.resetForm(); } catch (error) { console.error("Gagal simpan:", error.response?.data || error); let msg = 'Gagal simpan.'; if (error.response?.data?.message) { msg = error.response.data.message; if(error.response.data.errors) { msg += "\nDetails:\n" + Object.entries(error.response.data.errors).map(([f, m]) => `${f}: ${m.join(', ')}`).join("\n"); } } this.showNotification(msg, 'error'); } finally { this.isLoading = false; } },
    editTeamMember(member) { this.resetForm(); this.isEditing = true; const memberCopy = JSON.parse(JSON.stringify(member)); const initialForm = this.getInitialForm(); const bioData = memberCopy.bio_data && typeof memberCopy.bio_data === 'object' ? memberCopy.bio_data : initialForm.bio_data; let socialLinksArray = []; if (memberCopy.social_links && typeof memberCopy.social_links === 'object' && !Array.isArray(memberCopy.social_links)) { Object.keys(memberCopy.social_links).forEach(p => { if (memberCopy.social_links[p]) { socialLinksArray.push({ platform: p, url: memberCopy.social_links[p] }); } }); } else if (Array.isArray(memberCopy.social_links)) { socialLinksArray = memberCopy.social_links; } this.form = { ...initialForm, ...memberCopy, bio_data: { ...initialForm.bio_data, ...bioData, prestasi: Array.isArray(bioData.prestasi) ? bioData.prestasi : [] }, social_links: socialLinksArray, }; this.selectedPhotoFile = null; this.selectedHeaderFile = null; this.photoPreviewUrl = null; this.headerPreviewUrl = null; if (this.$refs.photoInput) this.$refs.photoInput.value = ''; if (this.$refs.headerInput) this.$refs.headerInput.value = ''; this.$el.closest('.overflow-y-auto')?.scrollTo({ top: 0, behavior: 'smooth' }); },
    confirmDelete(id) { if (window.confirm('Yakin hapus?')) { this.deleteTeamMember(id); } },
    async deleteTeamMember(id) { try { await axios.delete(`${API_URL}/${id}`); this.showNotification('Berhasil dihapus.'); await this.fetchTeamMembers(); if (this.isEditing && this.form.id === id) { this.resetForm(); } } catch (error) { console.error("Gagal hapus:", error); this.showNotification('Gagal hapus.', 'error'); } },
    resetForm() { this.isEditing = false; this.form = this.getInitialForm(); this.selectedPhotoFile = null; this.selectedHeaderFile = null; this.photoPreviewUrl = null; this.headerPreviewUrl = null; if (this.$refs.photoInput) this.$refs.photoInput.value = ''; if (this.$refs.headerInput) this.$refs.headerInput.value = ''; },
    addSocialLink() { if (!Array.isArray(this.form.social_links)) { this.form.social_links = []; } this.form.social_links.push({ platform: 'instagram', url: '' }); },
    removeSocialLink(index) { if (Array.isArray(this.form.social_links)) { this.form.social_links.splice(index, 1); } },
    getPlaceholderForPlatform(platform) { const placeholders = { instagram: 'https://instagram.com/username', facebook: 'https://facebook.com/username', email: 'email@contoh.com', whatsapp: 'https://wa.me/628123456789', tiktok: 'https://tiktok.com/@username', twitter: 'https://twitter.com/username', youtube: 'https://youtube.com/channel/UC...', linkedin: 'https://linkedin.com/in/username', telegram: 'https://t.me/username' }; return placeholders[platform] || 'Masukkan URL...'; },
  },
  mounted() { this.fetchTeamMembers(); },
};
</script>
