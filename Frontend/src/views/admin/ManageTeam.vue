<template>
  <div>
    <div v-if="notification.show" class="fixed top-5 right-5 z-50 transition-all duration-300" :class="notification.type === 'success' ? 'bg-emerald-500' : 'bg-red-500'">
        <div class="text-white text-sm font-bold px-4 py-3 rounded-lg shadow-lg">
    ...
    <div v-if="angkatanStore.activeAngkatan" class="bg-blue-100 text-blue-800 px-4 py-2 rounded mb-4 border border-blue-200">
        <i class="fas fa-info-circle mr-2"></i>
        Anda sedang mengelola data untuk: <strong>{{ angkatanStore.activeAngkatan.name }} ({{ angkatanStore.activeAngkatan.year_period }})</strong>
    </div>
          ...
          <tbody>
            <tr v-for="member in teamMembers" :key="member.id" class="hover:bg-slate-50">
             ...
             <tr v-if="teamMembers.length === 0">
                <td colspan="4" class="text-center p-8 text-blueGray-500">
                    Tidak ada anggota di angkatan ini.
                </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  data() {
    return {
      angkatanStore: useAngkatanStore(), // Gunakan Store
      teamMembers: [],
      isEditing: false,
      isLoading: false,
  ...
  // --- INI YANG PENTING: Watch perubahan angkatan ---
  watch: {
    'angkatanStore.selectedId': {
        ...
        immediate: true // Jalankan saat pertama kali juga
    }
  },
  methods: {
    getInitialForm() {
    ...
    async fetchTeamMembers() {
      if (!this.angkatanStore.selectedId) return; // Jangan fetch kalau belum ada angkatan

      try {
        const response = await axios.get(API_URL, {
            params: {
                angkatan_id: this.angkatanStore.selectedId // Kirim ID angkatan
            }
        });
    ...
    async submitForm() {
      ...
      // Ambil ID dari store dan masukkan ke form data
      if (this.angkatanStore && this.angkatanStore.selectedId) {
      ...
      try {
        ...
        await this.fetchTeamMembers(); // Refresh tabel dengan filter yang sama
        this.resetForm();
      } catch (error) {
    ...
    // ... (Metode lain: editTeamMember, confirmDelete, deleteTeamMember, resetForm, dll. SAMA SEPERTI SEBELUMNYA)
  ...
  // Mounted sudah digantikan oleh watch immediate: true
  // mounted() {
  //   this.fetchTeamMembers();
  // },
};
